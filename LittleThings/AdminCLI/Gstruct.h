 ...
 *	angle					angle
	...
	chain					a chain list
		...
		.creat(CHAIN_STRUCT)		creat a chain list
		.add(CHAIN_STRUCT)		add a point
		.read(int)			read the value.............	(CHAIN_STRUCT)
		.del(int)			delete a point
		.length()			get length.................	(int)
 */
int gcdl(int m,int n)
{
	if(m%n==0)
		return n;
	else
...
struct fraction
{
	...
	int rof() 
	{
		do{
			mom*=10;
			son*=10;
		}while(!(ceil(mom)==floor(mom) && ceil(son)==floor(son)));
		if(mom<0)	{mom*=-1;son*=-1;}
		int g=gcdl(mom,son);
		if(g==1)
			return g;
		else
	...
	fraction plus(fraction p2)
	{
		fraction sum;
		sum.mom=1;sum.son=1;
		int g=gcdl(p2.mom,mom);
		g=mom*p2.mom/g;
		sum.son*=g/p2.mom+g/mom;
		sum.mom=g;
		sum.rof();
		return sum;
	}
...
ostream& operator<<(ostream& os,fraction& F){os<<F.son<<"/"<<F.mom;return os;}
...
class chain
{
	private:
		struct point
		{
			CHAIN_STRUCT value;
			point *next=NULL;
			point(CHAIN_STRUCT x):
				value(x),next(NULL){
			}
		}*head,*tail;
		CHAIN_STRUCT read(int num,point hed)
		{
			if(num==1)	return hed.value;
			return read(num-1,*hed.next);
		}
		int length(point hed)
		{
			if(hed.next==NULL) return 1;
			return 1+length(*hed.next);
		}
	public:
		void creat(CHAIN_STRUCT Value)
		{
			head=new point(Value);
			tail=head;
		}
		int length()
		{
		...
		void add(CHAIN_STRUCT Value)
		{
			point* temp=new point(Value);
			tail->next=temp;
			tail=tail->next;
		}
		CHAIN_STRUCT read(int num)
		{
			return read(num,*head);
		}
		void del(CHAIN_STRUCT data)
		{
			point *p;
			p=head;
			if(tail!=NULL)
			...
			{
				while(p->next!=NULL&&p->next->value!=data)
					p=p->next;
				if(p->next==NULL);
				else
				{
					point* deleteNode=p->next;
